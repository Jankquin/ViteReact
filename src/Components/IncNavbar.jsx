...
const IncNavbar = () => {
    ...
    return (
        <>
            <div className="bg-white dark:bg-slate-900 shadow fixed w-full top-0 p-3 z-50">
                <div className="container mx-auto">
                    <div className="flex justify-center">
                        ...
                        <div className="lg:w-5/12 md:w-8/12 w-full">
                            <form className="dark:bg-slate-700 bg-slate-100 relative rounded">
                                {LabelSearch ? 
                                    <div className="absolute flex text-slate-500 text-sm font-medium uppercase h-full w-full">
                                ...
                                <input className="bg-transparent text-slate-500 rounded text-sm relative outline-none w-full z-10 p-3" type="text" value={Search} onChange={Filter}/>
                            </form>
                        </div>

                        <div className="lg:w-3/12 md:w-2/12 md:bg-transparent bg-slate-100 flex justify-end">
                            <button className="flex dark:hover:bg-slate-700 text-slate-500 hover:bg-slate-200 rounded-full px-3 mr-5">
                                <i className="bi-moon-stars-fill text-xl self-center"></i>
                            </button>
                            <button className="flex dark:hover:bg-slate-700 text-slate-500 hover:bg-slate-200 rounded-full px-3" onClick={Modal.Alert == true ? (event) => setModal({Alert: false}) : (event) => setModal({Alert: true})}>
            ...
            {Search.length !== 0 && (
                <div className="bg-slate-600/80 fixed w-full h-full top-0 left-0 z-30 animate-fadeIn">
                    <div className="container md:px-0 px-3 py-2 mx-auto mt-16">
                        <div className="flex justify-center">
                            <div className="lg:basis-5/12 md:basis-8/12 basis-full bg-white shadow-lg rounded-sm p-5 overflow-hidden block">
                                {Found.length !== 0 ?
                                    <>
                                        {Found.slice(0, 5).map((doc, key) => {
                                            return (
                                                <a href={`/${doc.Id}`} key={doc.Id} className="flex bg-slate-100 hover:bg-slate-200 hover:text-indigo-400 text-slate-600 group rounded px-4 py-2 mb-1" onClick={(event) => setModal({Alert: false})}>
                                                    ...
                                                    <span>
                                                        ...
                                                        <div className="flex">
                                                            <i className="bi-eye-fill group-hover:text-indigo-400 text-slate-400 self-center mr-3" />
                                                            <div className="group-hover:text-indigo-400 text-slate-400 self-center text-xs">{doc.View}</div>
                                                        </div>
                                                    </span>
                                                </a>    
                                            );
                                ...
                                <div className="text-slate-400 text-xs mt-5">Powered by : Valonime</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed bg-slate-600/80 flex top-0 left-0 w-full h-full z-40">
                    <div className="bg-white lg:w-5/12 w-10/12 rounded self-center mx-auto p-5 animate-fadeIn">
                        <div className="flex justify-between mb-5">
                            <button className="text-slate-600 text-xl font-medium" onClick={(event) => setModal({Alert: false})}>Valonime</button>
                            <button className="hover:bg-slate-200 rounded-full px-2 py-1" onClick={(event) => setModal({Alert: false})}>
                                <i className="bi-x"/>
                            </button>
                        </div>

                        <Link to="/" className="flex bg-slate-200/50 hover:text-indigo-400 text-slate-600 group rounded px-4 py-2 mb-1" onClick={(event) => setModal({Alert: false})}>
                        ...
                        <Link to="/more" className="flex bg-slate-200/50 hover:text-indigo-400 text-slate-600 group rounded px-4 py-2 mb-1" onClick={(event) => setModal({Alert: false})}>
